...
- Fnなどのホールド中だけマイクを使用
- 右Commandなど、通常入力とは別の設定可能なキーを押している間にCodexへの音声質問を録音
...
Codex質問では、音声をMac上で文字起こしした後、その質問文をローカルのCodex CLIからOpenAIへ送信します。音声データ自体は送信しません。

24時間録音、システム音声取得、会議管理、Todo生成は含みません。
...
- Apple Silicon Mac
- Xcode 26 / Swift 6.3
- Codex質問を使う場合はCodex CLIがインストール済みで、利用可能なアカウントにログイン済みであること
...
音声認識はMac上で実行されます。通常の音声入力は外部へ送信されません。Codex質問を使った場合だけ、認識後の質問文がCodex CLIを介してOpenAIへ送信されます。入力履歴はローカルに保存されます。詳細は[PRIVACY.md](PRIVACY.md)を参照してください。
